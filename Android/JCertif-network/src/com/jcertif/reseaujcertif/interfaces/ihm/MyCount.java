...
public class MyCount extends Activity {
	...
	private TextView login_txt, password_txt, name_txt, firstname_txt, berth_day_txt, mail_txt, payes_txt, ville_txt, presentation_txt, inscription, title_activity;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        title_activity.setText("Mon compte");
        inscription.setText("Modifier");
        ...
        new Thread(){
        	...
        	public void run(){
        		...
        		MyCount.this.runOnUiThread(new Runnable() {
	...
	protected void returnMenuPrincipal() {
		...
		new Thread(){
			...
			public void run(){
				...
				Intent intent = new Intent(MyCount.this, Principale.class);           		           		
...
//    	date.startAnimation(animation);
	...
	private void setData() {
        login.setText(jCertifManager.getCurrentUser().getLogin());
        password.setText(jCertifManager.getCurrentUser().getPassword());
        name.setText(jCertifManager.getCurrentUser().getName());
        firstname.setText(jCertifManager.getCurrentUser().getFirstname());
        berth_day.setText(jCertifManager.getCurrentUser().getDate_naissance());
        mail.setText(jCertifManager.getCurrentUser().getEmail());
        payes.setText(jCertifManager.getCurrentUser().getPayes());
        ville.setText(jCertifManager.getCurrentUser().getVille());
        presentation.setText(jCertifManager.getCurrentUser().getDescription());
	}
	...
	private void desableReturn(){
		inscription.setBackgroundResource(R.drawable.return_off);
		returnMenuPrincipal();
		isSelected = false;
	}
	...
	private void inscription() {
		...
		if(login.getText().toString().length()==0 || password.getText().toString().length()==0){
			...
			if((mail.getText().toString().length()>0 && mail.getText().toString().contains("@")) || (mail.getText().toString().length()==0)){
				user = jCertifManager.getCurrentUser();
				user.setAdmin(jCertifManager.getCurrentUser().isAdmin());
				user.setLogin(login.getText().toString());
				user.setPassword(login.getText().toString());
				...
				user.setStatus(jCertifManager.getCurrentUser().isStatus());
				user.setPhoto_url(jCertifManager.getCurrentUser().getPhoto_url());
				...
				alertDialog.setTitle("Modifier ce compte");			
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						new Thread(){
							...
							public void run(){
								...
								reponse = new ParsingUsers(JCertifManager.getInstance()).updateUser(user);
	...
	public void saveModification(User user, boolean rep){
		...
		if(rep){
			showWelcomMsg("Modification effectuée avec succés");
			...
			showWelcomMsg("Problème lors de la modification");
